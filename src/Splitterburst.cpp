...
struct Splitterburst : Module {
	...
	int steps = round(clamp(params[STEPS_PARAM].getValue(),1.f,64.f));
	int multiply = round(clamp(params[MULTI_PARAM].getValue(),1.f,64.f));
...
struct SplitterburstWidget : ModuleWidget {
	SplitterburstWidget(Splitterburst* module) {
		...
		addChild(createLight<MediumLight<GreenLight>>(mm2px(Vec(16, 35)), module, Splitterburst::ON_LIGHT));
		...
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(18, 64)), module, Splitterburst::GATE_INPUT));
